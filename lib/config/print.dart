...
class PrintPosPage extends StatefulWidget {
  const PrintPosPage({Key? key}) : super(key: key);

  @override
  State<PrintPosPage> createState() => _PrintPosPageState();
}

class _PrintPosPageState extends State<PrintPosPage> {
  ...
  //   final Image? imagePrint = decodeImage(buffer);
  ...
  Future _printReceiveTest() async {
    List<int> bytes = [];
    final ByteData data =
        await rootBundle.load("assets/images/entebbemlogo.png");
    // Xprinter XP-N160I assets/images/kccagrayscale.png
    ...
    if (data.lengthInBytes > 0) {
      ...
      img.Image originalImg =
          img.copyResize(decodedImage, width: 130, height: 130);
      // fills the original image with a white background
      // img.fill(originalImg, mage.color: img.ColorRgb8(255, 255, 255));
      var padding = (originalImg.width - thumbnail.width) / 2;

      //insert the image inside the frame and center it
      img.drawImage(originalImg, thumbnail, dstX: padding.toInt());
    ...
    bytes += generator.text('Confirm');
    // bytes += generator.qrcode('google.com');
